...
Event.observe(window, 'load', function() {
  ...
  ["/images/parties/add_a_new_person-pressed.gif",
   ...
   "/images/sidebar_nubbin-long.png"].each(function(path) {
...
var Categories = {
  create: function(options) {
    ...
    if (name = prompt("Enter a name for the new category:")) {
      new Ajax.Request("/categories", { 
        parameters: Object.extend({ "category[name]": name }, options.parameters || {}) 
      });
      return true;
    }
  },
...
var Cookie = {
  ...
  Store: {
    get: function() {
      var path = $A(arguments), key = path.pop();
      return this._read(path, key);
    },
    
    set: function() {
      var path = $A(arguments), value = path.pop(), key = path.pop();
      return this._write(path, key, value);
    },
    
    remove: function() {
      var path = $A(arguments), key = path.pop();
      return this._write(path, key, null);
    },
    
    _store: function() {
      return (Cookie.get('_store') || '{}').evalJSON();
    },
    
    _read: function(path, key) {
      return path.inject(this._store(), function(o, k) { return o[k] || {} })[key];
    },
    
    _write: function(path, key, value) {
      var store = this._store();
      path.inject(store, function(o, k) { return o[k] = o[k] || {} })[key] = value;
      Cookie.set('_store', Object.toJSON(store));
      return value;
    }
  }
};
...
var CreditCards = {
  validate: function() {
    var card = $('credit_card_number');

    if (card.value.length == 15 || card.value.length == 16) { 
      return true;
    } else {
      alert('The credit card number must be 15 or 16 digits');
      return false;
    }
  }
}
...
var EditTabs = {
  ...
  submit: function() {
    if(!EditTabs.validate()) return false;
...
var Files = {
  ...
  removePending: function(path, id) {
  ...
  findFileSelector: function(path, id) {
    return $(id).getElementsBySelector('input.file_selectors').select(function(file_selector) { return file_selector.value == path; }).first();
  },

  removeFileSelector: function(path, id) {
    this.findFileSelector(path, id).up().remove();
  },
  
  updatePendingAttachments: function(id) {
    new Ajax.Request('/files/update_pending', 
      { parameters: this.pendingFilesAsParameters(id) + "&id=" + id, method: 'put' });
  },
  ...
  pendingFilesAsParameters: function(id) {
    return $(id).getElementsBySelector('input.file_selectors').select(function(file_selector) {
      ...
      return "files[]=" + escape(file_selector.value);
    }).join("&");
  }
}
...
var Kases = {
  ...
  removeNewCase: function(selector) {
    ...
    if (value != "new_kase" && !value.match(/^\d+$/)) {
  ...
  selected: function(selector, options) {
    ...
    switch(kase_id) {
      case 'new_kase':
        Kases.refreshPermissions(options);
        Kases.add(selector);
        break;
      case '':
        Kases.removeNewCase(selector);
        Kases.refreshPermissions(options);
        break;
      default:
        Kases.removeNewCase(selector);
        options.kase_id = kase_id;
        Kases.refreshPermissions(options);
    }
  },
  ...
  refreshPermissions: function(options) {
    new Ajax.Request(options.presentation_url, { parameters: 
      ...
      (options.kase_id ? '&kase_id=' + options.kase_id : '') +
      (options.note_id ? '&recording_id=' + options.recording_id : '')
    });
  },
  
  preselect: function(company_id) {
    for (i = 0; i < $('kases_options').length; i++) {
      if ($('kases_options').options[i].value == company_id.toString()) {
        $('kases_options').selectedIndex = i;
      }
    }
  },
...
var Layout = {
  swapWithScreenBody: function(element_to_swap, field_to_preselect) {
...
var Notes = {
  submit: function(form, notes_url) {
    ...
    if (Files.hasPendingAttachments(form)) {
      ...
      new Ajax.Request(notes_url, { parameters: Form.serialize(form) });
      return false;
    }
  },

  validate: function(form) {
    var has_files = Files.hasPendingAttachments(form) || Files.hasExistingAttachments(form);
...
var People = {
  ...
  liveSearch: function(search_url, context) {
    if (context == "parties") {
      $('parties').setStyle({opacity: '.5'});
    } else if (context == "latest") {
      $('parties_for_latest').setStyle({opacity: '.5'});
    } else if (context == "duplicates") {
      $('duplicate_parties').setStyle({opacity: '.5'});
    }

    $('live_search_spinner').show();

    var options = arguments[2] || {};
    var url = search_url + '?context=' + context + '&term=' + $('live_search').value;
    if(options['exclude']) {
      url = url + "&exclude=" + options['exclude'];
    }

    new Ajax.Request(url,
      { onSuccess: function() { 
          $('live_search_spinner').hide();

          if (context == "parties") {
            $('parties').setStyle({opacity: '1'});
          } else if (context == "latest") {
            $('parties_for_latest').setStyle({opacity: '1'});
          } else if (context == "duplicates") {
            $('duplicate_parties').setStyle({opacity: '1'});
          }
        }
      }
    )
...
var Permissions = {
  ...
  addSelector: function(selector_parent_id) {
    this.update(selector_parent_id);
    $(selector_parent_id).getElementsByClassName('add_viewer_selector').last().addClassName('busy');
  },

  update: function(selector_parent_id) {
    new Ajax.Request('/permissions/update', { parameters: Form.serialize($(selector_parent_id)), method: 'get' });
  }
}
...
var ReturnTo = {
  remember: function(source) {
    ...
    if (source != destination) {
      Cookie.Store.set("return_to", {});
      Cookie.Store.set("return_to", source, destination);
    }
  }
};
...
var Signup = {
  proposeSubdomain: function() {
    var companyName = document.getElementById('signup_company_first_name').value;
    var subdomain   = companyName.replace(/[^a-zA-Z0-9]/g, "").toLowerCase();

    if (document.getElementById('signup_account_subdomain').value == '') {
      document.getElementById('signup_account_subdomain').value = subdomain;
    }
    
    Signup.updateSiteUrl();
  },
  
  updateSiteUrl: function() {
    $('site_url_name').innerHTML = $('signup_account_subdomain').value + ".hiqhrisehq.com";
  },

  enableSignup: function(checkbox) {
  ...
  toggleFollowupForm: function() {
    ...
    if (dialog.visible()) {
      ...
      dialog.visualEffect('blindDown', { 
        duration: 0.3,
        afterFinish: function() { $('followup_body_task').focus() }
      });
    }
  },
...
var Users = {
  ...
  showOpenId: function(container) {
    ...
    $(container).getElementsByClassName('password_entry').invoke('hide');
    $(container).getElementsByClassName('open_id_entry').invoke('show');
    $(container).getElementsByClassName('identity_url').first().focus();
  },
  
  hideOpenId: function(container) {
    if ($('link_to_open_id')) $('link_to_open_id').show();
    if ($('link_to_normal_login')) $('link_to_normal_login').hide();
    $(container).getElementsByClassName('password_entry').invoke('show');
    $(container).getElementsByClassName('open_id_entry').invoke('hide');
    $(container).getElementsByClassName('identity_url').first().value = "";
    $(container).getElementsByClassName('user_name').first().focus();
  }
}
